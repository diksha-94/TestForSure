...
Server.prototype._envelope = function(body, includeTimestamp) {
    ...
    var xml = "<?xml version=\"1.0\" encoding=\"utf-8\"?>" +
        //FCE Customization - START - Change namespace to SOAP 1.2
        //"<soap:Envelope xmlns:soap=\"http://schemas.xmlsoap.org/soap/envelope/\" " +        
        "<soap:Envelope xmlns:soap=\"http://www.w3.org/2003/05/soap-envelope\" " +
        //FCE Customization - END - Change namespace to SOAP 1.2
        encoding +
    ...
    //FCE Customization - START - Append tns: to all response nodes
    //look for opentag , and change it to <tns:
    //console.log('Body >> ' + body)
    var newBody = ''
    var skip = false
    for (var i = 0; i < body.length; i++) {
        if (body[i] === '<') {
            if (body.length >= i + 1 && body[i + 1] === '/') {
                newBody += '</tns:'
                skip = true
            } else {
                newBody += '<tns:'
    ...
    //Add tns:Errors element
    var addErrors = '<tns:Errors/>'
    var updatedBody2 = newBody
    if (updatedBody2.indexOf('<tns:Errors>') === -1) {
        if (newBody.indexOf('</tns:CreateFrameworkContractResponse>') > -1) {
            var updatedBody1 = newBody.substring(0, newBody.indexOf('</tns:CreateFrameworkContractResponse>'))
            updatedBody2 = updatedBody1 + addErrors + '</tns:CreateFrameworkContractResponse>'
        }
    }
    //FCE Customization - END - Append tns: to all response nodes
